...
export class WorktreeResolver {
     ...
     */
    async getMainRepo(worktreePath) {
    ...
    async resolveByName(name) {
        ...
        const matches = worktrees.filter(w => w.branchName.includes(name) || w.path.includes(name));
        if (matches.length === 0) {
            throw new Error(`No worktree found with name: ${name}`);
        }
        if (matches.length > 1) {
            const branchNames = matches.map(w => w.branchName).join(', ');
            throw new Error(`Multiple worktrees match "${name}": ${branchNames}. Please be more specific.`);
        }
        return matches[0];
    }
    /**
...
//# sourceMappingURL=WorktreeResolver.js.map