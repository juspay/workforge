...
export class DiffDisplay {
     ...
     */
    printUnchangedSummary(unchanged) {
        this.logger.newline();
    ...
    printFooter(diff) {
        ...
        if (totalChanges === 0) {
            ...
            if (addedCount > 0) {
                this.logger.listItem(chalk.green(`${addedCount} variable${addedCount === 1 ? '' : 's'} to be added`));
            }
            if (modifiedCount > 0) {
                this.logger.listItem(chalk.yellow(`${modifiedCount} variable${modifiedCount === 1 ? '' : 's'} to be modified`));
            }
            if (removedCount > 0) {
                this.logger.listItem(chalk.red(`${removedCount} variable${removedCount === 1 ? '' : 's'} to be removed`));
            }
            this.logger.listItem(chalk.dim(`${unchangedCount} variable${unchangedCount === 1 ? '' : 's'} unchanged`));
            this.logger.newline();
            this.logger.bold(chalk.cyan(`Total changes: ${totalChanges}`));
        }
        this.logger.newline();
    }
    /**
     ...
     */
    showNoChanges() {
        this.logger.box('No environment variable changes detected.\nSource and target .env files are identical.', 'info');
    }
    /**
     ...
     */
    formatQuotedValue(value, quoteType) {
