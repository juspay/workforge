...
export class CloseCommand {
    ...
    async run() {
        try {
                    ...
                    if (totalSelected > 0) {
                        ...
                        if (confirmed) {
                            ...
                            if (syncResult.success) {
                                ...
                                syncPerformed = true;
                                // Log audit
                                const auditOp = this.auditLogger.createAuditOperation(syncResult, `Worktree: ${worktree.branchName}`, `Main: ${path.basename(mainRepo)}`, backupCreated);
            ...
            const config = this.configManager.load();
            const shouldDeleteBranch = this.options.deleteBranch || (config.preferences.autoDeleteBranch && !worktree.isMainRepo);
            if (shouldDeleteBranch && worktree.branchName && worktree.branchName !== 'HEAD') {
                this.logger.newline();
                this.logger.step(4, 5, 'Deleting branch...');
                const cleanupResult = await this.branchCleaner.cleanup(worktree.branchName, mainRepo, safetyResult, this.options.force);
    ...
    displaySafetyResults(result) {
        ...
        if (result.warnings.length > 0) {
            for (const warning of result.warnings) {
                if (warning.includes('uncommitted') ||
     ...
     * Display summary
     */
    displaySummary(branchName, syncPerformed, backupCreated, branchDeleted) {
