...
async function main() {
    await yargs(hideBin(process.argv))
        ...
        .version('3.0.0')
        // ===== CREATE COMMAND =====
        .command('create', 'Create a new Git worktree', (yargs) => {
        return yargs
            .option('type', {
            alias: 't',
            type: 'string',
        ...
        const config = {
            ...
            base: argv.base,
            yes: argv.yes,
            ticketId: argv.ticket
        };
        const command = new CreateCommand(config);
        await command.run();
    })
        // ===== CLOSE COMMAND =====
        .command('close [path]', 'Close a worktree with intelligent environment sync', (yargs) => {
        return yargs
            .positional('path', {
            type: 'string',
        ...
        // ===== SYNC-ENV COMMAND =====
        .command('sync-env', 'Sync environment variables between worktrees', (yargs) => {
        return yargs
            .option('from', {
            type: 'string',
        ...
        // ===== LIST COMMAND =====
        .command('list', 'List all worktrees', (yargs) => {
        return yargs
            .option('all', {
            alias: 'a',
            type: 'boolean',
        ...
        // ===== CLEANUP COMMAND =====
        .command('cleanup', 'Clean up old backups and logs', (yargs) => {
        return yargs
            .option('older-than', {
            type: 'number',
