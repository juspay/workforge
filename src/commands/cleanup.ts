...
export class CleanupCommand {
  ...
  private showBackupPreview(backupsToDelete: BackupInfo[], totalBackups: number): void {
    ...
    for (const backup of toShow) {
      ...
      this.logger.listItem(
        `${chalk.dim(backup.fileName)} - ${chalk.yellow(age)} (${size})`
      );
    }
   ...
   * Display summary
   */
  private displaySummary(deletedCount: number, failedCount: number, remainingCount: number): void {
    this.logger.subheader('Summary:');

    if (deletedCount > 0) {
      this.logger.listItem(
        chalk.green(`${deletedCount} backup${deletedCount === 1 ? '' : 's'} deleted`)
      );
    }

    if (failedCount > 0) {
      this.logger.listItem(
        chalk.red(`${failedCount} backup${failedCount === 1 ? '' : 's'} failed to delete`)
      );
    }

    this.logger.listItem(
      chalk.dim(`${remainingCount} backup${remainingCount === 1 ? '' : 's'} remaining`)
    );
