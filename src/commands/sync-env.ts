...
export class SyncEnvCommand {
  private options: SyncEnvOptions;
  private configManager: ConfigManager;
  private logger: Logger;

  // Core components
  private syncTargetResolver: SyncTargetResolver;
  ...
  async run(): Promise<void> {
    try {
      ...
      const syncResult = await this.envSyncer.sync(
        targets.sourcePath,
        targets.targetPath,
        diff,
        decision
  ...
  private displaySummary(syncResult: SyncResult, backupCreated: boolean): void {
    ...
    if (syncResult.addedCount > 0) {
      this.logger.listItem(
        chalk.green(`${syncResult.addedCount} variable${syncResult.addedCount === 1 ? '' : 's'} added`)
      );
    }
    ...
    if (syncResult.modifiedCount > 0) {
      this.logger.listItem(
        chalk.yellow(
          `${syncResult.modifiedCount} variable${syncResult.modifiedCount === 1 ? '' : 's'} modified`
        )
      );
    }
    ...
    if (syncResult.removedCount > 0) {
      this.logger.listItem(
        chalk.red(
          `${syncResult.removedCount} variable${syncResult.removedCount === 1 ? '' : 's'} removed`
        )
      );
    }
