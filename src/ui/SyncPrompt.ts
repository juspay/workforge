...
export class SyncPrompt {
   ...
   */
  private async promptAdded(diff: EnvDiff): Promise<string[]> {
    this.logger.newline();
   ...
   */
  private async promptModified(diff: EnvDiff): Promise<string[]> {
    this.logger.newline();
   ...
   */
  private async promptRemoved(diff: EnvDiff): Promise<string[]> {
    this.logger.newline();
   ...
   */
  private formatVariableChoice(key: string, value: string, type: 'added' | 'removed'): string {
   ...
   */
  private formatModifiedChoice(key: string, oldValue: string, newValue: string): string {
   ...
   */
  async confirmSync(decision: SyncDecision): Promise<boolean> {
    const totalChanges =
   ...
   */
  async askForBackup(): Promise<boolean> {
  ...
  async promptDirection(
    sourceLabel: string,
    targetLabel: string
  ): Promise<'source-to-target' | 'target-to-source' | 'cancel'> {
    this.logger.newline();
      ...
      if (totalSelected === 0) {
        ...
        if (decision.addedKeys.size > 0) {
          this.logger.listItem(
            chalk.green(`${decision.addedKeys.size} to add: `) +
        ...
        if (decision.modifiedKeys.size > 0) {
          this.logger.listItem(
            chalk.yellow(`${decision.modifiedKeys.size} to modify: `) +
        ...
        if (decision.removedKeys.size > 0) {
          this.logger.listItem(
            chalk.red(`${decision.removedKeys.size} to remove: `) +
